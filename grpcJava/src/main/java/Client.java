...
public class Client {
    ...
    private void run() {
        ManagedChannel ch = ManagedChannelBuilder.forAddress("localhost", 50080)
                .usePlaintext().build();
   ...
   private void square(ManagedChannel channel) {
        ...
        CalculatorRequest req = CalculatorRequest.newBuilder().setNumber(5).build();
  ...
  private void factorial(ManagedChannel channel) {
        CalculatorServiceGrpc.CalculatorServiceBlockingStub stub = CalculatorServiceGrpc.newBlockingStub(channel);

        CalculatorRequest req = CalculatorRequest.newBuilder().setNumber(5).build();

        CalculatorFactorialResponse res = stub.factorial(req);

        System.out.println("factorial " + res.getResult());
    }
    ...
    private void primeFactors(ManagedChannel channel) {
        ...
        stub.primeFactors(req).forEachRemaining(primeFactorResponse -> System.out.println("Prime Factors: " + primeFactorResponse.getResult()));
    }
